...
const Login = ({ onLogin }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const response = await fetch(
      `https://localhost:7128/pirates?name=${name}&ship=${shipName}`
    );
  ...
  return (
    <div className={styles.login__wrapper}>
      <form
        ...
        className={`login rounded-lg border-double border-4 border-lime-700 p-10 max-w-xl mx-auto ${styles.login__form}`}
      >
